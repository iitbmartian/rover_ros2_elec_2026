...
from all_interfaces.msg import DriveData, TelemetryData, ArmEndMotion
import serial
import struct
...
class UARTBridge(Node):
    def __init__(self):
        ...
        self.create_subscription(ArmEndMotion, 'arm_commands', self.linear_base_callback, 10)
    ...
    def drive_callback(self, msg):
        ...
        self.motor_values[:8] = drive

    def linear_base_callback(self, msg):
        linear = self.add_direction([msg.speed[2]], [msg.direction[2]])
        self.motor_values[8] = linear[0]
    #
    ...
    def send_uart(self):
        fmt = '>9H'  # tells uart payload frame type, 11 H (unsigned 16 bit) + 1 B (unsigned 8 bit)
